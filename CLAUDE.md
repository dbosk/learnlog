...
- `playing-lectures.nw` — a log the learner was given (a lecture's bundle; their own export stands in): put it in the project, `play -f`, `checkout --from` into the folder named after it, run and edit the copy, `play -c` resumes the lecture, `checkout --restore` takes the copy out
