...
(`commits.nw` holds the readers of the recorded format — including `parse_run_block` (keys incl. `directory`)/`parse_exception_line`, `IO_STREAM_PREFIXES` and `split_commit_detail`, which the exporter, the metrics module and the display layer all import; `progsnap2.nw` the export side; `render.nw` the display layer; `get_commit_detail` runs `git show --no-color` and takes `reorder=False` for callers that lay the parts out themselves):
